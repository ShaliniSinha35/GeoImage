...
const validationSchema = yup.object().shape({
  ...
  photos: yup
    ...
    .test('videoSize', 'Video size should not exceed 10 MB', async (value) => {
      if (value.length === 0) return true;

      const videoSize = await getVideoSize(value[0].uri);
      return videoSize <= 10 * 1024 * 1024; // 10 MB
    })
    .test('videoDuration', 'Video duration should be between 4 and 20 minutes', async (value) => {
      if (value.length === 0) return true;

      const videoDuration = await getVideoDuration(value[0].uri);
      return videoDuration >= 4 * 60 && videoDuration <= 20 * 60; // 4 minutes to 20 minutes
    }),
});
...
const HomeScreen = () => {
  ...
  const getVideoDuration = async (uri) => {
    try {
      const videoDuration = await Camera.getVideoInfoAsync(uri);
            ...
            {hasCameraPermission === null ? (
              ...
              <View style={styles.cameraContainer}>
                <Camera
                  ...
                  type={Camera.Constants.Type.back}
                />
                <TouchableOpacity onPress={takeVideo} style={styles.captureButton}>
                  <Ionicons name="camera" size={24} color="white" />
                </TouchableOpacity>
                <View style={{ flexDirection: "row" }}>
                  {videoArray.map((photo, index) => (
                    <View key={index} style={{ marginVertical: 10, marginRight: 5 }}>
                      <Image source={{ uri: photo.uri }} style={{ width: 80, height: 80 }} />
                    </View>
                  ))}
